...
def jbot_create_license():
    ...
    if product not in valid_products:
        return jsonify({"ok": False, "error": "Gecersiz product"}), 400
    ...
    try:
        ...
        cur.close()
        conn.close()
        return jsonify({"ok": True, "id": license_id, "license_key": license_key,
                        "product": product, "expires_at": expires_at.isoformat(), "note": note})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def jbot_verify():
    ...
    try:
        ...
        allowed = (lic_product == "full" or lic_product == product or
                   (lic_product == "rocpickup_autotarget" and product in ["rocpickup", "autotarget"]))
        if not allowed:
            cur.close(); conn.close()
            return jsonify({"ok": False, "error": "Bu lisans bu urunu kapsamiyor"}), 403
        if not stored_hwid:
            cur.execute("UPDATE jbot_licenses SET hwid = %s WHERE id = %s", (hwid, lic_id))
            conn.commit()
        elif stored_hwid != hwid:
            cur.close(); conn.close()
            return jsonify({"ok": False, "error": "Bu lisans baska bir bilgisayarda kayitli"}), 409
        cur.close(); conn.close()
        return jsonify({"ok": True, "product": lic_product, "expires_at": expires_at.isoformat()})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
